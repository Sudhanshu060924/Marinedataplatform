...
function Filters({
  ...
  const species = useMemo(
    () => Array.from(new Set(rows.map((r) => r["Species Name"]))).sort(),
    [rows],
  );
  const zones = useMemo(
          ...
          {rows.map((r, i) => (
            <tr key={i} className="border-t">
              ...
              <td className="px-3 py-2">
                {(() => {
                  const ll = getLatLng(r.Location);
                  return ll ? (
                    <a
                      className="text-primary hover:underline"
                      href={`https://www.google.com/maps?q=${ll.lat},${ll.lng}`}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {r.Location}
  ...
  return [...rows].sort((a, b) => {
    const va =
      key === "Date"
        ...
        : key === "Location"
          ? (getLatLng(a.Location)?.lat ?? 0)
          : (a as any)[key];
    const vb =
      key === "Date"
        ? new Date(b.Date).getTime()
        : key === "Location"
          ? (getLatLng(b.Location)?.lat ?? 0)
          : (b as any)[key];
    ...
    <div className="space-y-6">
      <div className="flex items-baseline justify-between">
        ...
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => downloadCsv(sorted, "marine-data.csv")}
          >
            Export CSV
          </Button>
          <Button asChild>
            <a href="/data-explorer/full">Open full view</a>
          </Button>
        </div>
      </div>
...
export function DataExplorerFull() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-baseline justify-between">
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">
          Data Explorer — Full View
        </h1>
        <div className="flex gap-2">
          <Button
            variant="outline"
            onClick={() => downloadCsv(sorted, "marine-data-full.csv")}
          >
            Download CSV
          </Button>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            All Results ({filtered.length.toLocaleString()})
          </CardTitle>
        </CardHeader>
        <CardContent>
